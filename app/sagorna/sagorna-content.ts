...
export const SAGORNA_CMS_STORAGE_KEY = "starkvis.cms.items.v3";
export const CMS_CHANGE_EVENT = "starkvis-cms-change";
const LEGACY_STORAGE_KEYS = [
  "starkvis.newsroom.drafts.v1",
  "starkvis.newsroom.scheduled.v1",
  "starkvis.newsroom.published.v1",
  "starkvis.newsroom.archived.v1",
  "starkvis.sagorna.items.v1",
  "starkvis.sagorna.collection.v1",
  "starkvis.cms.items.v2",
];
...
function loadRawItems(key: string) {
...
async function mirrorCmsItemsToServer(items: SagornaCollection) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  try {
    ...
    // Local editor state still remains the source of truth in the browser.
  }
}

function promoteScheduledItems(items: SagornaCollection) {
  const now = Date.now();
  let changed = false;
  ...
  return { items: next.sort((left, right) => left.createdAt - right.createdAt), changed };
}

function collectLegacyItems() {
  const merged: SagornaCollection = [];

  LEGACY_STORAGE_KEYS.forEach((key) => {
    const raw = loadRawItems(key);
    if (!raw) {
      return;
    }

    const statusFromKey: SagornaStatus =
      key.includes("drafts")
        ? "draft"
    ...
    normalizeSagornaItems(raw, statusFromKey).forEach((item) => {
      merged.push({
        ...item,
        status: item.status === "archived" ? "archived" : statusFromKey,
        publishMode: statusFromKey === "scheduled" ? "scheduled" : "now",
      });
    });
  });

  const deduped = new Map<string, SagornaItem>();
  merged.forEach((item) => {
...
export function loadCmsItems() {
  ...
  const hasCurrentCollection = window.localStorage.getItem(SAGORNA_CMS_STORAGE_KEY) !== null;
  const raw = loadRawItems(SAGORNA_CMS_STORAGE_KEY);
  const items = raw ? normalizeSagornaItems(raw) : [];
  const source = hasCurrentCollection ? items : collectLegacyItems();
  const promoted = promoteScheduledItems(source);

  if (!hasCurrentCollection && promoted.items.length > 0) {
    saveCmsItems(promoted.items);
  ...
  if (promoted.items.length > 0) {
    void mirrorCmsItemsToServer(promoted.items);
  }

  return promoted.items;
}

export function loadCmsItemsSafe() {
  return loadCmsItems();
}

export function loadPublishedSagornaItemsSafe() {
  return loadCmsItemsSafe().filter((item) => item.status === "published");
}

export function loadArchivedSagornaItemsSafe() {
  return loadCmsItemsSafe().filter((item) => item.status === "archived");
}

export function saveCmsItems(items: SagornaCollection) {
  if (typeof window === "undefined") {
    return;
  }

  const next = ensureUniqueSlugs([...items].sort((left, right) => left.createdAt - right.createdAt));
  window.localStorage.setItem(SAGORNA_CMS_STORAGE_KEY, JSON.stringify(next));
  void mirrorCmsItemsToServer(next);
  window.dispatchEvent(new Event(CMS_CHANGE_EVENT));
}
